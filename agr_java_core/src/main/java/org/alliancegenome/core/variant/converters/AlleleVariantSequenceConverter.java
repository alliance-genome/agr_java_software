...
public class AlleleVariantSequenceConverter {
    ...
    public List<AlleleVariantSequence> convertVariantContext(VariantContext ctx, SpeciesType speciesType, String[] header, Map<String, List<Allele>> alleleMap, List<String> matched) throws Exception {
        ...
        if(returnDocuments.size() == 0) {
            returnDocuments = convertContextToSearchDocument(ctx, header, speciesType);
    ...
    private List<AlleleVariantSequence> getVariantsIfMatchedWithLTP(VariantContext ctx, Map<String, List<Allele>> alleleMap, String[] header, List<String> matched) throws Exception {
        ...
        for (htsjdk.variant.variantcontext.Allele a : ctx.getAlternateAlleles()) {
            ...
            List<TranscriptLevelConsequence> list = getConsequences(ctx, a.getBaseString(), header);
            
            String hgvsNomenclature = list.stream()
                    .findFirst()
            ...
            if (alleleMap != null && !alleleMap.isEmpty() && alleleMap.containsKey(hgvsNomenclature.trim())) {
                ...
                for (Allele al : alleles) {
                    returnDocuments.addAll(translateToNewAlleleVariantSequence(al, hgvsNomenclature, "true"));
    ...
    private List<AlleleVariantSequence> translateToNewAlleleVariantSequence(Allele a, String hgvsg, String matchedWithHtp) {
    ...
    private List<AlleleVariantSequence> convertContextToSearchDocument(VariantContext ctx, String[] header, SpeciesType speciesType) throws Exception {
