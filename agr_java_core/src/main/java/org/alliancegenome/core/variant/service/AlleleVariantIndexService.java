...
public class AlleleVariantIndexService {
    ...
    public JsonResultResponse<Allele> getJsonResult(List<SearchHit> searchHits,  Pagination pagination, String id)  {
        ...
        for(SearchHit searchHit: searchHits) {
            System.out.println("HIT:"+searchHit.getSourceAsString());
    ...
    public JsonResultResponse<Allele> getSearchResponse(String id, Pagination pagination) throws IOException {
       ...
       //   buildAggregations(srb);
        ...
        srb.trackTotalHits(true);
        //   System.out.println("QUERY:"+srb.query());
    ...
    public BoolQueryBuilder buildBoolQuery(String id, Pagination pagination){
        BoolQueryBuilder queryBuilder = new BoolQueryBuilder();
        queryBuilder.must(QueryBuilders.termQuery("geneIds.keyword", id));
           ...
           if (filterValueMap != null) {
               for (Map.Entry e : filterValueMap.entrySet()) {
                   ...
                   if (e.getKey().toString().equalsIgnoreCase("symbol")) {
                       queryBuilder.filter(QueryBuilders.matchPhraseQuery("allele.symbol", e.getValue().toString().toLowerCase().trim()).analyzer(""));
                   }
                   if (e.getKey().toString().equalsIgnoreCase("synonym")) {
                       queryBuilder.filter(QueryBuilders.matchPhraseQuery("allele.synonyms", e.getValue().toString().toLowerCase().trim()));
                   }
...
*/
    ...
    public void   buildAggregations(SearchSourceBuilder searchSourceBuilder){
        List<String> aggregationFields=  new ArrayList<>(Arrays.asList("variantType", "hasDisease","allele.hasPhenotype",
                "allele.variants.transcriptLevelConsequence.impact",
                "allele.variants.transcriptLevelConsequence.polyphenPrediction",
                "allele.variants.transcriptLevelConsequence.siftPrediction",
                "allele.variants.transcriptLevelConsequence.sequenceFeatureType",
                "allele.variants.transcriptLevelConsequence.transcript.name",
                "allele.gene.symbol","allele.variants.transcriptLevelConsequence.location",
                "allele.variants.transcriptLevelConsequence.molecularConsequences",
                "alterationType"));
    ...
    public Map<String, List<String>> getAggregations(String id){
        ...
        if (searchResponse != null && searchResponse.getAggregations()!=null) {
            ...
            Terms impact = searchResponse.getAggregations().get("impact");
            aggregations.put("filter.impact", new ArrayList<>());
            for (Terms.Bucket b : impact.getBuckets()) {
                if (!b.getKey().toString().equals(""))
                    aggregations.get("filter.impact").add((String) b.getKey());
            }
            Terms polyphenPrediction = searchResponse.getAggregations().get("polyphenPrediction");
            aggregations.put("filter.variantPolyphen", new ArrayList<>());
            for (Terms.Bucket b : polyphenPrediction.getBuckets()) {
                if (!b.getKey().toString().equals(""))
                    aggregations.get("filter.variantPolyphen").add((String) b.getKey());
            }
            Terms siftPrediction = searchResponse.getAggregations().get("siftPrediction");
            aggregations.put("filter.variantSift", new ArrayList<>());
            for (Terms.Bucket b : siftPrediction.getBuckets()) {
                if (!b.getKey().toString().equals(""))
                    aggregations.get("filter.variantSift").add((String) b.getKey());
            }
            Terms sequenceFeatureType = searchResponse.getAggregations().get("sequenceFeatureType");
            aggregations.put("filter.sequenceFeatureType", new ArrayList<>());
            for (Terms.Bucket b : sequenceFeatureType.getBuckets()) {
                if (!b.getKey().toString().equals(""))
                    aggregations.get("filter.sequenceFeatureType").add((String) b.getKey());
            }
            Terms transcriptName = searchResponse.getAggregations().get("transcriptName");
            aggregations.put("filter.transcriptName", new ArrayList<>());
            for (Terms.Bucket b : transcriptName.getBuckets()) {
                if (!b.getKey().toString().equals(""))
                    aggregations.get("filter.transcriptName").add((String) b.getKey());
            }
            Terms location = searchResponse.getAggregations().get("location");
            aggregations.put("filter.variantLocation", new ArrayList<>());
            for (Terms.Bucket b : location.getBuckets()) {
                if (!b.getKey().toString().equals(""))
