...
public interface FilterFunction<Entity, FilterValue> {
	...
	static boolean contains(String entityName, String value) {
		...
		Set<Boolean> resultSet = Arrays.stream(token).map(val -> entityName.trim().toLowerCase().contains(val.toLowerCase())).collect(Collectors.toSet());
	...
	static boolean fullMatchMultiValueOR(String entity, String value, String delimiter) {
		...
		List<String> cleanedValues = Arrays.stream(tokenList).map(s -> s.toLowerCase().trim()).collect(Collectors.toList());
