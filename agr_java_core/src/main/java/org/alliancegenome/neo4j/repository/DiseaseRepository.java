...
public class DiseaseRepository extends Neo4jRepository<DOTerm> {
	...
	private static Map<String, Set<String>> closureMapGO;
	private static Map<String, Set<String>> closureMapUberon;
	private static Map<String, Set<String>> closureMapUberonChild;
	private static Map<String, Set<String>> closureMap;
	private static Map<String, Set<String>> doClosureChildMap;
	...
	public List<DiseaseEntityJoin> getAllDiseaseAnnotationsModelLevel() {
		...
		String cypher = "MATCH diseaseJoin=(disease:DOTerm)--(dej:DiseaseEntityJoin)-[:EVIDENCE]->(pubJoin:PublicationJoin)<-[:ASSOCIATION]-(publication:Publication), " 
			...
			+ "OPTIONAL MATCH ecoCodes=(pubJoin:PublicationJoin)-[:ASSOCIATION]-(:ECOTerm) " + "OPTIONAL MATCH allele=(agm:AffectedGenomicModel)--(a:Allele) " + "OPTIONAL MATCH alleleGene=(agm:AffectedGenomicModel)--(a:Allele)--(:Gene) "
			+ "OPTIONAL MATCH str=(agm:AffectedGenomicModel)--(:SequenceTargetingReagent)--(:Gene) " + "OPTIONAL MATCH crossRef=(dej:DiseaseEntityJoin)--(:AffectedGenomicModel)-[:CROSS_REFERENCE]->(:CrossReference) "
			+ "OPTIONAL MATCH condition=(dej:DiseaseEntityJoin)--(:ExperimentalCondition)--(:ZECOTerm) " + "return diseaseJoin, model, crossRef, alleleGene, modelSpecies, allele, str, ecoCodes, condition ";

		Iterable<DiseaseEntityJoin> joins = query(DiseaseEntityJoin.class, cypher);
		List<DiseaseEntityJoin> allJoins = StreamSupport.stream(joins.spliterator(), false).collect(Collectors.toList());
	...
	public Set<String> getAllDiseaseWithAnnotationsKeys() {
		String query = "MATCH (term:DOTerm)-[q:ASSOCIATION]-(dej:DiseaseEntityJoin) WHERE term.isObsolete='false' " + " RETURN term.primaryKey";
	...
	public Map<String, Set<String>> getClosureMapping() {
		...
		// closure
		String cypher = "MATCH (diseaseParent:DOTerm)<-[:IS_A_PART_OF_CLOSURE]-(disease:DOTerm) where diseaseParent.isObsolete = 'false' " + " return diseaseParent.primaryKey as parent, disease.primaryKey as child ";

		List<Closure> cls = getClosures(cypher);
		closureMap = cls.stream().collect(groupingBy(Closure::getParent, mapping(Closure::getChild, Collectors.toSet())));
		return closureMap;
	}
	...
	public Map<String, Set<String>> getClosureMappingUberon() {
		...
		List<Closure> cls = getClosures(cypher);
		closureMapUberon = cls.stream().collect(groupingBy(Closure::getParent, mapping(Closure::getChild, Collectors.toSet())));

		closureMapUberonChild = cls.stream().collect(groupingBy(Closure::getChild, mapping(Closure::getParent, Collectors.toSet())));
		return closureMapUberon;
	}
	...
	public Map<String, Set<String>> getClosureMappingGO() {
		...
		List<Closure> cls = getClosures(cypher);
		closureMapGO = cls.stream().collect(groupingBy(Closure::getParent, mapping(Closure::getChild, Collectors.toSet())));
	...
	private List<Closure> getClosures(String cypher) {
		...
		return StreamSupport.stream(result.spliterator(), false).map(stringObjectMap -> {
	...
	public Map<String, Set<String>> getDOClosureChildMapping() {
		...
		// closure
		String cypher = "MATCH (diseaseParent:DOTerm)<-[:IS_A_PART_OF_CLOSURE]-(disease:DOTerm) where diseaseParent.isObsolete = 'false' " + " return diseaseParent.primaryKey as parent, disease.primaryKey as child order by disease.name";

		List<Closure> cls = getClosures(cypher);
		doClosureChildMap = cls.stream().collect(groupingBy(Closure::getChild, mapping(Closure::getParent, Collectors.toSet())));
		return doClosureChildMap;
	}
	...
	public List<DOTerm> getAgrDoSlim() {
		...
		String cypher = "MATCH (disease:DOTerm) where disease.subset =~ '.*" + DOTerm.HIGH_LEVEL_TERM_LIST_SLIM + ".*' RETURN disease order by disease.name";

		Iterable<DOTerm> joins = query(cypher);

		doAgrDoList = StreamSupport.stream(joins.spliterator(), false).collect(Collectors.toList());
	...
	public Set<ECOTerm> getEcoTerms(List<PublicationJoin> publicationEvidenceCodeJoin) {
		...
		String cypher = "MATCH p=(pubEvCode:PublicationJoin)-[:ASSOCIATION]->(ev:ECOTerm) " + "where pubEvCode.primaryKey in [" + ids + "]" + "RETURN p";

		Iterable<ECOTerm> joins = query(ECOTerm.class, cypher);

		return StreamSupport.stream(joins.spliterator(), false).collect(Collectors.toSet());
	}
	...
	private void populateAllPublicationJoins() {
		...
		String cypher = "MATCH p=(pubEvCode:PublicationJoin)" + " OPTIONAL MATCH (pubEvCode:PublicationJoin)-[:ASSOCIATION]->(ev:ECOTerm) " + "RETURN pubEvCode.primaryKey as " + pubCodeID + ", collect(ev) as " + ecoName;
		...
		result.forEach(resultMap -> {
			...
			if (resultMap.get(ecoName) != null) {
				// if the cypher query does not have values for collect(ev) the OGM generates an
				// empty array Objects[]
				if (resultMap.get(ecoName) instanceof ArrayList) {
					names = (List<ECOTerm>) resultMap.get(ecoName);
					/*
					 * else log.
					 * debug("Collection of ECOterms was not an ArrayList for pubEvCodeJoin ID : " +
					 * id);
					 */
				}
			}
		...
		// Iterable<PublicationJoin> joins = neo4jSession.query(PublicationJoin.class,
		// cypher, new HashMap<>());
	...
	public List<ECOTerm> getEcoTerm(List<PublicationJoin> joins) {
		...
		joins.stream().filter(join -> ecoTermMap.get(join.getPrimaryKey()) != null).forEach(publicationJoin -> terms.addAll(ecoTermMap.get(publicationJoin.getPrimaryKey())));
		return terms;
	}

	public DOTerm getShallowDiseaseTerm(String id) {

		String cypher = "MATCH (disease:DOTerm) WHERE disease.primaryKey = $primaryKey	  " + " RETURN disease ";
	...
	public UBERONTerm getShallowUberonTerm(String id) {
		...
		String cypher = "MATCH (disease:UBERONTerm) WHERE disease.primaryKey = $primaryKey	  " + " RETURN disease ";
	...
	public GOTerm getGOTerm(String id) {
		...
		String cypher = "MATCH (disease:GOTerm) WHERE disease.primaryKey = $primaryKey	  " + " RETURN disease ";
	...
	// Convenience method to populate the evidence codes on the publicationJoins
	// object
	public void populatePublicationJoins(List<PublicationJoin> joins) {
		...
		joins.stream().filter(join -> ecoTermMap.get(join.getPrimaryKey()) != null).forEach(join -> join.setEcoCode(ecoTermMap.get(join.getPrimaryKey())));
	}
	...
	public Set<DiseaseEntityJoin> getAllDiseaseEntityGeneJoins() {
		...
		/*
		 * cypher += " AND disease.primaryKey in ['DOID:3146'] "; cypher +=
		 * " AND gene.primaryKey= 'WB:WBGene00022781' ";
		 */
		// cypher += " AND diseaseEntityJoin.primaryKey =
		// 'FB:FBgn0030343DOID:1838is_implicated_in' ";
		// cypher += " AND disease.primaryKey in ['DOID:9952','DOID:14501'] ";
		// cypher += " AND dej.primaryKey = 'MGI:1891259DOID:0110188IS_IMPLICATED_IN' ";
		// cypher += " OPTIONAL MATCH eco
		// =(pubEvCode:PublicationJoin)-[:ASSOCIATION]->(ecoTerm:ECOTerm)";
		...
		Set<DiseaseEntityJoin> allDiseaseEntityJoins = StreamSupport.stream(joins.spliterator(), false).collect(Collectors.toSet());
	...
	public Set<DiseaseEntityJoin> cleanupDiseaseEntityJoins(Set<DiseaseEntityJoin> allDiseaseEntityJoins) {
		Set<DiseaseEntityJoin> allDiseaseEntityJoinsStripped = allDiseaseEntityJoins.stream().filter(join -> join.getPublicationJoins() != null).collect(Collectors.toSet());
			...
			diseaseEntityJoin.getPublicationJoins().forEach(publicationJoin -> {
				if (publicationJoin.getAlleles() != null) {
					publicationJoin.getAlleles().forEach(allele -> {
						allele.setDiseaseEntityJoins(allele.getDiseaseEntityJoins().stream().filter(phenotypeEntityJoin1 -> phenotypeEntityJoin1.getDisease().getPrimaryKey().equals(diseaseEntityJoin.getDisease().getPrimaryKey())).collect(Collectors.toList()));
					});
				}
			});
		});
	...
	public Set<DiseaseEntityJoin> getAllDiseaseAlleleEntityJoins() {
		String cypher = "MATCH p=(disease:DOTerm)-[:ASSOCIATION]-(diseaseEntityJoin:DiseaseEntityJoin)-[:EVIDENCE]->(pubEvCode:PublicationJoin)-[:ASSOCIATION]-(publication:Publication),"
			+ "			  p1=(diseaseEntityJoin:DiseaseEntityJoin)<-[:ASSOCIATION]-(feature:Feature)-[:CROSS_REFERENCE]->(crossReference:CrossReference), " + "			  p2=(feature:Feature)-[:FROM_SPECIES]->(:Species) ";
		cypher += " where disease.isObsolete = 'false' ";
		// cypher += " AND disease.primaryKey in
		// ['DOID:0050144','DOID:0110599','DOID:0050545'] ";
		// cypher += " AND disease.primaryKey in ['DOID:14503'] and feature.primaryKey =
		// 'WB:WBVar00251972'";
		// cypher += " AND feature.primaryKey = 'ZFIN:ZDB-ALT-040720-42' AND
		// disease.primaryKey = 'DOID:0080422' ";
		// cypher += " OPTIONAL MATCH eco
		// =(pubEvCode:PublicationJoin)-[:ASSOCIATION]->(ecoTerm:ECOTerm)";
			...
			diseaseEntityJoin.getPublicationJoins().forEach(publicationJoin -> {
				if (publicationJoin.getModels() != null) {
					publicationJoin.getModels().forEach(model -> {
						...
						// using the setter allele.setDiseaseEntityJoin and as the model object has many
						// of them they
						// are overridden
	...
	public List<DiseaseEntityJoin> getAllDejRecords(String id, String doID) {
		...
		return joins.stream().filter(join -> join.getDisease().getPrimaryKey().equals(doID)).collect(Collectors.toList());
	}
	...
	public Map<String, List<DiseaseEntityJoin>> getAllDejRecords() {
		...
		// "where gene.primaryKey = 'ZFIN:ZDB-GENE-040426-1716' AND phenotype.primaryKey
		// = 'ball increased size, abnormal' " +
		// "where gene.primaryKey = 'SGD:S000004966' AND phenotype.primaryKey =
		// 'increased chemical compound accumulation' " +
			+ "where node:Allele OR node:AffectedGenomicModel "
			// "AND disease.primaryKey = 'DOID:0080422' " +
			+ "OPTIONAL MATCH baseLevel=(dej:DiseaseEntityJoin)--(:ExperimentalCondition)-[:ASSOCIATION]->(:ZECOTerm) " + "return p0, baseLevel ";
		Iterable<DiseaseEntityJoin> dejJoins = query(DiseaseEntityJoin.class, cypherBaseLevelDEJ);
		dejAgmMap = StreamSupport.stream(dejJoins.spliterator(), false).filter(diseaseEntityJoin -> diseaseEntityJoin.getAllele() != null).collect(groupingBy(join -> join.getAllele().getPrimaryKey()));
		Map<String, List<DiseaseEntityJoin>> dejModelMap = StreamSupport.stream(dejJoins.spliterator(), false).filter(diseaseEntityJoin -> diseaseEntityJoin.getModel() != null).collect(groupingBy(join -> join.getModel().getPrimaryKey()));
	...
	public Long getTotalDiseaseCount(String geneID, Pagination pagination, boolean empiricalDisease) {
		...
		String baseCypher = "MATCH p0=(disease:DOTerm)--(diseaseEntityJoin:DiseaseEntityJoin)-[:EVIDENCE]-(pubEvCode:PublicationJoin), " + "			   p1=(publications:Publication)--(pubEvCode:PublicationJoin)--(evidence:ECOTerm), "
