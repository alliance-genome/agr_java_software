...
sub map_coordinates {
  ...
  warn "[TRACE] Mapper.map_coordinates id=$id start=$start end=$end strand=$strand type=$type from=$self->{from} to=$self->{to}\n";
  ...
  for my $r (@result) {
    if ($r->isa('Bio::EnsEMBL::Mapper::Coordinate')) {
      warn "[TRACE]   -> Coord id=" . $r->id . " start=" . $r->start . " end=" . $r->end . " strand=" . $r->strand . "\n";
    } elsif ($r->isa('Bio::EnsEMBL::Mapper::Gap')) {
      warn "[TRACE]   -> Gap start=" . $r->start . " end=" . $r->end . "\n";
    }
  }
