...
sub cdna_start {
    ...
    unless (exists $self->{cdna_start}) {
        ...
        my $vf = eval { $self->base_variation_feature || $self->variation_feature };
        my $vf_pos = $vf ? ($vf->seq_region_start . ":" . $vf->seq_region_end) : '?';
        warn "[TRACE] BTV.cdna_start tr=$tr_id vf=$vf_pos cdna_start=" . (defined $self->{cdna_start} ? $self->{cdna_start} : 'undef') . " cdna_end=" . (defined $self->{cdna_end} ? $self->{cdna_end} : 'undef') . "\n";
    }
...
sub cds_start {
    ...
    unless (exists $self->{cds_start}) {
        ...
        my $vf = eval { $self->base_variation_feature || $self->variation_feature };
        my $vf_pos = $vf ? ($vf->seq_region_start . ":" . $vf->seq_region_end) : '?';
        warn "[TRACE] BTV.cds_start tr=$tr_id vf=$vf_pos exon_phase=$exon_phase cds_start=" . (defined $self->{cds_start} ? $self->{cds_start} : 'undef') . " cds_end=" . (defined $self->{cds_end} ? $self->{cds_end} : 'undef') . "\n";
    }
...
sub translation_start {
    ...
    unless (exists $self->{translation_start}) {
        ...
        my $vf = eval { $self->base_variation_feature || $self->variation_feature };
        my $vf_pos = $vf ? ($vf->seq_region_start . ":" . $vf->seq_region_end) : '?';
        warn "[TRACE] BTV.translation_start tr=$tr_id vf=$vf_pos translation_start=" . (defined $self->{translation_start} ? $self->{translation_start} : 'undef') . " translation_end=" . (defined $self->{translation_end} ? $self->{translation_end} : 'undef') . "\n";
    }
