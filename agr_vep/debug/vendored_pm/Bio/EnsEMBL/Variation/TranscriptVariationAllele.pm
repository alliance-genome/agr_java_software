...
sub peptide {
  ...
  my $tr_id = eval { $self->transcript->stable_id } || '?';
  my $allele = eval { $self->variation_feature_seq } || '?';
  warn "[TRACE] TVA.peptide tr=$tr_id allele=$allele peptide=" . (defined $self->{peptide} ? $self->{peptide} : 'undef') . "\n";
  return $self->{peptide};
}
...
sub codon {
  ...
  my $tr_id = eval { $self->transcript->stable_id } || '?';
  my $allele = eval { $self->variation_feature_seq } || '?';
  warn "[TRACE] TVA.codon tr=$tr_id allele=$allele codon=" . (defined $self->{codon} ? $self->{codon} : 'undef') . "\n";
  return $self->{codon};
}
...
sub hgvs_protein {
  ...
  unless($hgvs_notation) {
    ...
    my $tr_id = eval { $self->transcript->stable_id } || '?';
    my $allele = eval { $self->variation_feature_seq } || '?';
    warn "[TRACE] TVA.hgvs_protein tr=$tr_id allele=$allele result=undef reason=no_peptides\n";
    return undef;
  }

  ##### String formatting
  my $result = $self->_get_hgvs_protein_format($hgvs_notation, $prediction_format, $convert_to_three_letter);
  my $tr_id = eval { $self->transcript->stable_id } || '?';
  my $allele = eval { $self->variation_feature_seq } || '?';
  warn "[TRACE] TVA.hgvs_protein tr=$tr_id allele=$allele result=" . (defined $result ? $result : 'undef') . "\n";
  return $result;
}
...
sub _get_hgvs_protein_format {
  ...
  if( $hgvs_notation->{ref} eq $hgvs_notation->{alt} && $hgvs_notation->{type} ne "fs" && $hgvs_notation->{type} ne "ins"){
    warn "[TRACE] _get_hgvs_protein_format syn-branch: ref=" . $hgvs_notation->{ref} . " alt=" . $hgvs_notation->{alt} . " type=" . $hgvs_notation->{type} . " start=" . $hgvs_notation->{start} . "\n";
    $hgvs_notation->{'hgvs'} .= $hgvs_notation->{ref} . $hgvs_notation->{start} . "=";
    warn "[TRACE] _get_hgvs_protein_format syn-output: hgvs=" . $hgvs_notation->{'hgvs'} . "\n";
  }
...
sub _get_hgvs_protein_type {
  ...
  ### Run type checks on peptides if available
    warn "[TRACE] _get_hgvs_protein_type before-strip: ref=" . $hgvs_notation->{ref} . " alt=" . $hgvs_notation->{alt} . "\n";
    $hgvs_notation->{ref} =~ s/\*/X/;
    $hgvs_notation->{alt} =~ s/\*/X/;
    warn "[TRACE] _get_hgvs_protein_type after-strip: ref=" . $hgvs_notation->{ref} . " alt=" . $hgvs_notation->{alt} . "\n";
...
sub _get_hgvs_peptides {
  ...
  if($hgvs_notation->{type} eq "fs"){
    ...
    warn "[TRACE] _get_hgvs_peptides INS entry: start=" . $hgvs_notation->{start} . " end=" . $hgvs_notation->{end} . " ref=[" . ($hgvs_notation->{ref}//'') . "] alt=[" . ($hgvs_notation->{alt}//'') . "] orig_ref=[" . ($hgvs_notation->{original_ref}//'') . "]\n";

    ### Check if bases directly after insertion match inserted sequence
    $hgvs_notation = $self->_check_peptides_post_var($hgvs_notation);
    warn "[TRACE] _get_hgvs_peptides INS after-post-var: start=" . $hgvs_notation->{start} . " end=" . $hgvs_notation->{end} . " type=" . $hgvs_notation->{type} . "\n";
    ...
    warn "[TRACE] _get_hgvs_peptides INS after-dup-check: type=" . $hgvs_notation->{type} . " start=" . $hgvs_notation->{start} . " end=" . $hgvs_notation->{end} . " alt=[" . ($hgvs_notation->{alt}//'') . "]\n";
    ...
    warn "[TRACE] _get_hgvs_peptides INS after-surrounding: min=$min ref=[" . ($hgvs_notation->{ref}//'undef') . "]\n";
  ...
  if ( $convert_to_three_letter ){
      warn "[TRACE] _get_hgvs_peptides before-seq3: ref=" . $hgvs_notation->{ref} . " alt=" . $hgvs_notation->{alt} . " type=" . $hgvs_notation->{type} . "\n";
      ...
      warn "[TRACE] _get_hgvs_peptides after-seq3: ref=" . $hgvs_notation->{ref} . " alt=" . $hgvs_notation->{alt} . "\n";
  }
...
sub _get_surrounding_peptides {
  ...
  my $ref_trans  = $self->transcript_variation->_peptide();
  warn "[TRACE] _get_surrounding_peptides enter: ref_pos=$ref_pos orig_ref=[" . ($original_ref//'undef') . "] length=" . (defined $length ? $length : 'undef') . " ref_trans_len=" . length($ref_trans//'') . " ref_trans=[" . ($ref_trans//'') . "]\n";
  $ref_trans .= $original_ref
    if defined $original_ref && $original_ref =~ /^\*/;

  ## can't find peptide after the end
  if (length($ref_trans) <=  $ref_pos) {
    warn "[TRACE] _get_surrounding_peptides: returning undef because length(ref_trans)=" . length($ref_trans) . " <= ref_pos=$ref_pos\n";
    return;
  }
  ...
  warn "[TRACE] _get_surrounding_peptides: returning ref_string=[$ref_string]\n";
...
sub _check_for_peptide_duplication {
  ...
  warn "[TRACE] _check_for_peptide_duplication enter: ref_trans_len=" . length($reference_trans) . " ref_trans=[$reference_trans] alt=[" . ($hgvs_notation->{alt}//'') . "] start=" . $hgvs_notation->{start} . " preseq=[" . ($hgvs_notation->{preseq}//'') . "]\n";
  ...
  warn "[TRACE] _check_for_peptide_duplication compute: upstream_trans=[$upstream_trans] test_new_start=$test_new_start alt_len=" . length($hgvs_notation->{alt}) . "\n";
  ...
  if( (length($upstream_trans) >=  $test_new_start + length($hgvs_notation->{'alt'}) ) && $test_new_start  >=0){
    ...
    warn "[TRACE] _check_for_peptide_duplication test: test_seq=[$test_seq] alt=[" . $hgvs_notation->{alt} . "] match=" . ($test_seq eq $hgvs_notation->{alt} ? 'YES' : 'NO') . "\n";
    ...
    warn "[TRACE] _check_for_peptide_duplication skip: upstream_len=" . length($upstream_trans) . " test_new_start+alt_len=" . ($test_new_start + length($hgvs_notation->{alt})) . "\n";
  }
  return $hgvs_notation;
}
