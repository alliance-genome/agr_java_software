 ...
 * Creates AlleleVariantSequenceCuration documents instead of AlleleVariantSequence.
 */
@Slf4j
public class SourceDocumentCreationCuration extends Thread {
	...
	public SourceDocumentCreationCuration(String downloadPath, DownloadSource source, GeneDocumentCache geneCache) {
		...
		messageHeader = speciesType.getModName() + " [Curation] ";
	}
	...
	public void run() {
		...
		if (indexing) {
			...
			builder5 = BulkProcessor.builder((request, bulkListener) -> client5.bulkAsync(request, RequestOptions.DEFAULT, bulkListener), new BulkProcessor.Listener() {
				...
				@Override
				public void afterBulk(long executionId, BulkRequest request, Throwable failure) {
					log.error(messageHeader + "BulkProcessor5 Request Failure: " + failure.getMessage());
					failure.printStackTrace();
					System.exit(-1);
				}
			});
			...
			builder6 = BulkProcessor.builder((request, bulkListener) -> client6.bulkAsync(request, RequestOptions.DEFAULT, bulkListener), new BulkProcessor.Listener() {
				...
				@Override
				public void afterBulk(long executionId, BulkRequest request, Throwable failure) {
					log.error(messageHeader + "BulkProcessor6 Request Failure: " + failure.getMessage());
					failure.printStackTrace();
					System.exit(-1);
				}
			});
			...
			builder7 = BulkProcessor.builder((request, bulkListener) -> client7.bulkAsync(request, RequestOptions.DEFAULT, bulkListener), new BulkProcessor.Listener() {
				...
				@Override
				public void afterBulk(long executionId, BulkRequest request, Throwable failure) {
					log.error(messageHeader + "BulkProcessor7 Request Failure: " + failure.getMessage());
					failure.printStackTrace();
					System.exit(-1);
				}
			});
			...
			builder8 = BulkProcessor.builder((request, bulkListener) -> client8.bulkAsync(request, RequestOptions.DEFAULT, bulkListener), new BulkProcessor.Listener() {
				...
				@Override
				public void afterBulk(long executionId, BulkRequest request, Throwable failure) {
					log.error(messageHeader + "BulkProcessor8 Request Failure: " + failure.getMessage());
					failure.printStackTrace();
					System.exit(-1);
				}
			});
		...
		try {
			...
			TimeUnit.MILLISECONDS.sleep(15000);
			log.info(messageHeader + "Object Empty shutting down producers");
			...
			log.info(messageHeader + "Json Queue Empty shutting down bulk indexers");
			...
			if (gatherStats) {
				statsCollector.printOutput(speciesType.getModName() + "_curation");
			}
