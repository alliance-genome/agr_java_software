...
public class DiseaseCacheRepository {
    ...
    public PaginationResult<DiseaseAnnotation> getRibbonDiseaseAnnotations(List<String> geneIDs, String diseaseSlimID, Pagination pagination) {
        ...
        List<String> termIDs = DiseaseService.getDiseaseParents(diseaseSlimID);
        List<DiseaseAnnotation> slimDiseaseAnnotationList = new ArrayList<>();

        // filter by slim ID
        if (CollectionUtils.isNotEmpty(termIDs)) {
            termIDs.forEach(termID -> {
                List<DiseaseAnnotation> list = fullDiseaseAnnotationList.stream()
                        .filter(diseaseAnnotation -> diseaseAnnotation.getParentIDs().contains(termID))
                        .collect(Collectors.toList());
                slimDiseaseAnnotationList.addAll(list);
            });
        }
        //filtering
