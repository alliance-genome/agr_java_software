...
public class DiseaseAnnotationToTdfTranslator {
	...
	public String getAllRowsForGenesAndAlleles(List<DiseaseAnnotation> diseaseAnnotationsGenes, List<DiseaseAnnotation> diseaseAnnotationsAlleles, List<DiseaseAnnotation> diseaseAnnotationsModels) {
...
*/
			...
			for (org.alliancegenome.curation_api.model.entities.DiseaseAnnotation da : annotation.getPrimaryAnnotations()) {
				if (CollectionUtils.isNotEmpty(da.getConditionRelations())) {
					List<ConditionRelation> conditionModifiers = da.getConditionRelations().stream().filter(conditionRelation -> conditionRelation.getConditionRelationType() != null)
						.filter(conditionRelation -> conditionRelation.getConditionRelationType().getName().contains("has_condition") || conditionRelation.getConditionRelationType().getName().contains("induced") || conditionRelation.getConditionRelationType().getName().contains("ameliorated") || conditionRelation.getConditionRelationType().getName().contains("exacerbated")).toList();
	...
	private DiseaseDownloadRow getBaseDownloadRow(DiseaseAnnotation annotation, PublicationJoin join, Gene homologousGene) {
		...
		if (CollectionUtils.isNotEmpty(annotation.getProviders()) && annotation.getProviders().size() == 1) {
			final Map<String, CrossReference> crossReferenceMap = annotation.getProviders().get(0);
			String sourceProvider = crossReferenceMap.get("sourceProvider").getName();
			if (crossReferenceMap.size() == 2) {
				sourceProvider += " via ";
